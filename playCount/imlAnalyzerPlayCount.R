...
png(file = "~/dropbox/thesis/playCount/out_png/week_all.png")
for(i in 1:7){
	if(i>1){
...
for(i in 1:7){
		wdayName = switch(i, "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" )
                png(paste(file = "~/dropbox/thesis/playCount/out_png/week_",as.character(i), "_",wdayName, ".png", sep = ""))
...
for(i in 1:length(table(iml[,2]))){
	...
	targetGenre = iml[,1][iml[,2] == targetGenreName]
        ...
        ggsave(paste("~/dropbox/thesis/playCount/out_pdf/","genre_",i,"_",targetGenreName ,".pdf", sep=""), plot, family="Japan1GothicBBB")
}
