...
void testApp::setup(){
    ...
    myFont2.loadFont("MyriadPro-Regular.otf", 30);
    myFontForTimeLabel.loadFont("Ricty-Regular.ttf", 10);
    myFontJapanese.loadFont("Ricty-Regular.ttf", 10);
    myFontJapaneseLarge.loadFont("Ricty-Regular.ttf", 14);
    drawingSongInfoFont.loadFont("FOT-MatissePro-EB.otf", 18);
    ...
    myControlPanel.loadFont("Ricty-Bold.ttf", 12);
    myControlPanel.addPanel("コントロールパネル", 5);
//    vector<string> modeStr;
//    modeStr.push_back("developer");
//    modeStr.push_back("public");
...
//  myControlPanel.removeObject(string xmlName);
    ...
    myControlPanel.addMultiToggle("被験者を選択", "csvOwner", 0, csvOwnerList);
    
    myControlPanel.setWhichColumn(1);
    vector<string> timeUnitStr;
    timeUnitStr.push_back("24時間");
    timeUnitStr.push_back("1週間");
    myControlPanel.addMultiToggle("時間単位", "timeUnit", 1, timeUnitStr);
    myControlPanel.addSlider("topRange", "topRange", 5, 1, 10, true);
    
    vector<string> graphType;
    graphType.push_back("円グラフ");
    graphType.push_back("棒グラフ");
    myControlPanel.addMultiToggle("グラフの形状を選択", "graphType", 0, graphType);
    myControlPanel.addToggle("ratioMap", "ratioMap", false);
    myControlPanel.addToggle("topRangeRatio", "topRangeRatio", true);
    myControlPanel.addToggle("showLarge", "showLarge", false);
    myControlPanel.addToggle("showDetailLabel", "showDetailLabel", false);
...
//    myControlPanel.addLabel("press ^ or v --- change csvOwner");
    ...
    myControlPanel.addSlider("corAbsThreshold", "corAbsThreshold", 0.8, 0, 1, false);

    vector<string> weekOrHourStr;
    weekOrHourStr.push_back("week");
    weekOrHourStr.push_back("hour");
    myControlPanel.addMultiToggle("weekOrHour", "weekOrHour", 0, weekOrHourStr);
    ...
    for (int i = 0; i< csvOwnerList.size(); i++) {
        myControlPanel.addPanel("weekTimeSetting_"+csvOwnerList[i], 4);
        myControlPanel.setWhichPanel(i+2);
        myControlPanel.setSliderWidth(1000);
    ...
    for (int i = 0; i<csvOwnerList.size(); i++) {
        ...
        instantOfVector.loadFile(ofToDataPath(ofToString("csv/") + dirCSV.getName(i).c_str() + ofToString("/of.csv")));
        csvOfVector.push_back(instantOfVector);
    }
...
//    }
    ...
    setCurrentTopRangeCorVectorHour();
    
    
    
    //卒展
//    ofImage instantImage;
//    for (int i = 0; i < 5; i++) {
//        instantImage.loadImage("texture_" + ofToString(i) + ".png");
//        texture.push_back(instantImage);
//    }
    
    animationSwitch = false;
    animationButtonImg.loadImage("animation.png");
    animationButtonPos.set(850, 60);
    imgStable.loadImage("toga_logo_white.png");
    imgRotate.init(imgStable, 0);
//    playerDrawingAnimation.loadSound("square_4k_10ms.wav");
    playerDrawingAnimation.loadSound("square_8k_5ms_fadeout_-6db.wav");
    playerDrawingAnimation.setMultiPlay(true);
    printf("setupDone!\n");
}
    ...
    if (myControlPanel.getSelectedPanel() == 1) {
        ...
        if (myControlPanel.getValueI("sortType") == 0) {
            ...
            for (int i = 0; i<csvOwnerList.size(); i++) {
                ...
                myFontJapanese.drawString(csvOwnerList[i], 0, myFontForTimeLabel.stringHeight(csvOwnerList[i]));
                ofPopStyle();
                ofPushMatrix();
                ofPushMatrix();
                ofTranslate(70, 0);
        ...
        if (myControlPanel.getValueI("weekOrHour") == 0) {
            //week
            //曜日名を描画
            for (int i = 0; i<7; i++) {
                ofPushMatrix();
            ...
            //テーブルの設定
            myTable.setup();
            myTable.setFont(myFontForTimeLabel);
            myTable.setBoxSize(100, 15);
            myTable.setEnableHighlihgt(myControlPanel.getValueF("corAbsThreshold"), true);
            ...
            ofPopStyle();
            ofPopMatrix();
        }else if(myControlPanel.getValueI("weekOrHour") == 1){
            //hour
            //曜日名を描画
            for (int i = 0; i<=24; i++) {
                ofPushMatrix();
                ofTranslate(150+60*i, 15*csvOwnerList.size()+508);
                myFontForTimeLabel.drawString(ofToString(i), 50-myFontForTimeLabel.stringWidth(ofToString(i))/2.0f, 0);
                ofPopMatrix();
            }
            //TODO: 相関係数表を描画
            ofPushMatrix();
            ...
            myTable.setup();
            myTable.setFont(myFontForTimeLabel);
            myTable.setBoxSize(60, 15);
            myTable.setEnableHighlihgt(myControlPanel.getValueF("corAbsThreshold"), true);
            myTable.setTableSize(csvOwnerList.size(), 24);
            ofSetColor(127, 127, 127, 127);
            //テーブルの背景描画
            myTable.drawBase();
            ofSetColor(255, 255, 255, 127);
            ...
            ofSetColor(255, 255, 255, 255);
            for (int i=0; i<csvOwnerList.size(); i++) {
                myTable.drawVectorFloat(currentTopRangeCorVectorHour[i], whereAmI[i], true);
            }
            //テーブルの枠線描画
            ofSetColor(0, 0, 0, 255);
            myTable.drawFrame();
            ...
            ofSetColor(255, 255, 255, 255);
            myFontForTimeLabel.drawString("iTunesGenreRatio" , -125, -10);
            myFontForTimeLabel.drawString("Correlation Coefficient" , 500, -10);
            ofPopStyle();
            ofPopMatrix();
        }else{
            //something wrong
        ...
        if (myControlPanel.getValueI("timeUnit") == 1) {
            //hour and week
            ofPushStyle();
            ...
            if (myControlPanel.getValueI("graphType") == 0) {
                ...
                for (int i = 0; i<7; i++) {
                    ...
                    //曜日はじめ
                    ofPushMatrix();
                    ofTranslate(myControlPanel.getValueI(weekNameStringList[i]+"Start_"+csvOwnerList[myControlPanel.getValueI("csvOwner")])*ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/, 0);
                    ofPushStyle();
                    ofSetLineWidth(1);
                    ...
                    //曜日おわり
                    ofPushMatrix();
                    ofTranslate((myControlPanel.getValueI(weekNameStringList[i]+"End_"+csvOwnerList[myControlPanel.getValueI("csvOwner")])+1)*ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/, 0);
                    ofPushStyle();
                    ofSetLineWidth(1);
                ...
                //アニメーションするかどうか。（描画している音源の情報を文字列で描画）
                if (animationSwitch) {
                    animationFrameNum++;
                    drawingSongInfoFont.drawString(csvOfVector[myControlPanel.getValueI("csvOwner")].getString(animationFrameNum, 1) +
                                                   "\n" +
                                                   csvOfVector[myControlPanel.getValueI("csvOwner")].getString(animationFrameNum, 3) +
                                                   "\n" +
                                                   csvOfVector[myControlPanel.getValueI("csvOwner")].getString(animationFrameNum, 2) +
                                                   "\n" +
                                                   csvOfVector[myControlPanel.getValueI("csvOwner")].getString(animationFrameNum, 6)
                                                   , ofGetWidth()/2.0f, 70);
                    if (animationFrameNum >= csvOfVector[myControlPanel.getValueI("csvOwner")].numRows-1) {
                        animationSwitch = false;
                    }
                    //飛ばした棒グラフ
                    ofPushMatrix();
                    ...
                    //このフレームで描画する情報は、
                    //時間idは csvOfVector[myControlPanel.getValueI("csvOwner")].getInt(animationFrameNum, 5)
                    //ジャンルランクは　csvOfVector[myControlPanel.getValueI("csvOwner")].getInt(animationFrameNum, 4)
                    //ここに、描画している箇所のハイライトを行う。
                    ofPushStyle();
                    ofSetColor(255, 255, 255);
                    ofPopStyle();
                    
                    //入れる
                    animationVector[csvOfVector[myControlPanel.getValueI("csvOwner")].getInt(animationFrameNum, 5)][csvOfVector[myControlPanel.getValueI("csvOwner")].getInt(animationFrameNum, 4)-1] += 1;
                    //描画
                    //音楽データのアニメーション棒グラフを描画
                    for (int i = 0 ; i<168; i++) {
                        ofPushMatrix();
                        //棒グラフの横ポジションを変更
                        ofTranslate(ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/*i, ofGetHeight()*0.9);
                        float currentBarTop = 0;
                        for (int j = 0 ; j < csvGenreNameListVector[myControlPanel.getValueI("csvOwner")].numRows-1; j++) {//ランク1が0個目にあることを注意する。
                            //色を指定
                            ofPushStyle();
                            ofColor col;
                            if (j < 5) {
                                col.setHsb(genreCol[j].getHue(), 255, 255);
                            }else{
                                col.set(127, 127, 127);
                            }
                            ofSetColor(col);
                            //描画
                            ofRect(0, 0, ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/, -BAR_HEIGHT_CHANGER*animationVector[i][j]);
                            //ビームを描画
                            ofSetColor(255, 255, 255, 255);
                            ofSetLineWidth(1);
                            if (i == csvOfVector[myControlPanel.getValueI("csvOwner")].getInt(animationFrameNum, 5) && j == csvOfVector[myControlPanel.getValueI("csvOwner")].getInt(animationFrameNum, 4)) {
                                ofLine(ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*//2.0f, -BAR_HEIGHT_CHANGER*animationVector[i][j]/2.0f,
                                       ofGetWidth()-ofGetWidth()*0.15-ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/*i, ofGetHeight()*0.1-ofGetHeight()*0.9+currentBarTop);//描画中の四角から、円グラフの真ん中へ
                                playerDrawingAnimation.play();
                            }
                            ofPopStyle();
                            //棒グラフの縦ポジションを変更
                            currentBarTop += BAR_HEIGHT_CHANGER*animationVector[i][j];
                            ofTranslate(0, -BAR_HEIGHT_CHANGER*animationVector[i][j]);
                    ...
                    for (int i=0; i<24*7; i++) {
                        ...
                        for (int j=0; j<genreNum; j++) {
                            ...
                            ofSetColor(col);
                            //ここで棒グラフを描画
                            ofRect(0, 0, ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/, -BAR_HEIGHT_CHANGER*currentDistFromGraphTop);//最初の行がラベルなので１足す
                            //音楽ジャンルの分布を描画
                            if (myControlPanel.getValueB("ratioMap")) {
                                if (myControlPanel.getValueI("showLarge")) {
                                    ofRect(0, 20, ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/, 4*20*currentDistFromGraphTop/(float)graphTop[i]);
                                }else{
                                    ofRect(0, 20, ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/, 20*currentDistFromGraphTop/(float)graphTop[i]);
                                }
                            }
                            ofPopStyle();
                            ofPopMatrix();
                ...
                for (int i=0; i<7; i++) {
                    ...
                    if (myControlPanel.getValueB("showLarge")) {
                        myFontJapaneseLarge.drawString(str, ofGetWidth()/7.0f/2.0f-myFontJapaneseLarge.stringWidth(str)/2, myFontJapaneseLarge.stringHeight(str));
                    }else{
            ...
            ofPushMatrix();
            for (int i = 0; i< myControlPanel.getValueI("topRange"); i++) {
                ofSetColor(genreCol[i].r, genreCol[i].g, genreCol[i].b, 255);
                if (i>0) {
...
//            myVectorGraphics.arc(0, 0, checkedGraphRadius, 0, 360*currentOwnerCheckedTopRange/(float)checkedTotal);
            ...
            if (myControlPanel.getValueB("showDetailLabel")) {
            ...
            //曜日毎のベストレンジの閉める割合を出す。
            if (myControlPanel.getValueB("ratioMap") == true && myControlPanel.getValueB("showLarge") == true) {
                //do not show
            }else{
            ...
            hourBarVector.clear();
            for (int i = 0; i<24; i++) {
                vector<float> instantOneHourBar;
                for (int j = 0; j<csvGenreNameListVector[myControlPanel.getValueI("csvOwner")].numRows-1; j++) {
                    //累積になっているジャンルを、曜日に関係ない1時間にまとめる。
                    float instantSumHour = 0;
                    for (int k = 0; k<7; k++) {
                        instantSumHour += csvGraphDataMatrixVector[myControlPanel.getValueI("csvOwner")].getInt(j+1, 24*k+i+1);//最初の行と列がラベルなのでそれぞれ１足す;
                    }
                    instantOneHourBar.push_back(instantSumHour);
                }
                //一時間分の棒グラフの累積データを格納。
                hourBarVector.push_back(instantOneHourBar);
            }
            ...
            for (int i = 0; i<30; i++) {
                ofPushMatrix();
                ofTranslate(0, -10*(i+1)*BAR_HEIGHT_CHANGER/(float)2);
                ofPushStyle();
                ofPushStyle();
                ofSetLineWidth(1);
                ofSetColor(255, 255, 255, 127);
            ...
            //飛ばした棒グラフ
            ofPushMatrix();
            float convertedBeginBarPosX = beginBarPosX*7.0f-ofGetWidth()*floor(beginBarPosX*7.0f/(float)ofGetWidth());
            ofTranslate(convertedBeginBarPosX, 0);
            // j is weekNum: sunday is 0, monday is 1 ...saturday is 6.
            ...
            for (int i=0; i<24; i++) {
                currentDistFromGraphTop = graphTop[i];
                ofPushMatrix();
                ofTranslate(ofGetWidth()/(float)24*i, ofGetHeight()*0.9);
                float posY = 0;
                for (int j=0; j<csvGenreNameListVector[myControlPanel.getValueI("csvOwner")].numRows-1; j++) {
                    ofPushStyle();
                    ofColor col;
                    if (j<myControlPanel.getValueI("topRange")) {
                        col.setHsb(255/(float)myControlPanel.getValueI("topRange")*j, 255, 255);
                    }else{
                        col.set(127, 127, 127, 255);
                    }
                    
                    ofSetColor(col.r, col.g, col.b, 255);
                    //ここで棒を描画する。
                    ofRect(0, -posY, ofGetWidth()/(float)24, -BAR_HEIGHT_CHANGER/(float)2*hourBarVector[i][j]);
                    posY += BAR_HEIGHT_CHANGER/(float)2*hourBarVector[i][j];
                    ofPopStyle();
                }
                ofPopMatrix();
            }
            ...
            for (int i=0; i<24; i++) {
                ofPushMatrix();
                ofTranslate(ofGetWidth()/24.0f*i, ofGetHeight()*0.9);
                ofPushStyle();
                ofColor col;
                col.setHsb(255, 255, 255);
                ofSetColor(col);
                ofSetLineWidth(2);
                ofLine(ofGetWidth()/24.0f, 0, ofGetWidth()/24.0f, 10);
                if (myControlPanel.getValueB("ratioMap")) {
                    ofLine(ofGetWidth()/7.0f, 20, ofGetWidth()/7.0f, 10-1);
                }
                ...
                myFontForTimeLabel.drawString(str, -myFontForTimeLabel.stringWidth(str)/2, myFontForTimeLabel.stringHeight(str)*2);
                ofPopStyle();
                ofPopMatrix();
            }
            ofPopMatrix();
            //飛ばされた棒グラフ
            ofPushMatrix();
            ofTranslate(-ofGetWidth()+convertedBeginBarPosX, 0);
...
//            myVectorGraphics.arc(0, 0, checkedGraphRadius, 0, 360*currentOwnerCheckedTopRange/(float)checkedTotal);
    ...
    if (animationSwitch == false) {
        animationButtonImg.draw(animationButtonPos, animationButtonImg.width, animationButtonImg.height);
        imgStable.draw(    ofGetWidth()/2.0f-100, 50, 80, 80);
    }else{
        //棒グラフをアニメーションで描画中のとき
        imgRotate.draw(10, ofGetWidth()/2.0f-100+40, 90, 80, 80, 1);
    }
    //UI
    //コントロールパネルの大きさ設定
    switch (myControlPanel.getSelectedPanel()) {
        case 0:
            myControlPanel.setSize(640, 25*dirCSV.numFiles()-10);
            break;
        case 1:
            myControlPanel.setSize(640, 520);
            break;
        default:
            myControlPanel.setSize(640, 450);
            break;
    }
    //control panel
    myControlPanel.draw();
...
//    if (myControlPanel.getSelectedPanel() != 1) {
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("sundayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Sun- " + ofToString(myControlPanel.getValueI("sundayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'),       245, 155);
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("mondayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Mon- " + ofToString(myControlPanel.getValueI("mondayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'),       245, 155+43*1);
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("tuesdayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Tue- " + ofToString(myControlPanel.getValueI("tuesdayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'),     245, 155+43*2);
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("wednesdayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Wed- " + ofToString(myControlPanel.getValueI("wednesdayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'), 245, 155+43*3);
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("thursdayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Thu- " + ofToString(myControlPanel.getValueI("thursdayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'),   245, 155+43*4);
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("fridayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Fri- " + ofToString(myControlPanel.getValueI("fridayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'),       245, 155+43*5);
//        myFontForTimeLabel.drawString( ofToString(myControlPanel.getValueI("saturdayStart_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0') + " -Sat- " + ofToString(myControlPanel.getValueI("saturdayEnd_"+csvOwnerList[myControlPanel.getValueI("csvOwner")]), 3, '0'),   245, 155+43*6);
//    }
...
//    for (int i = 0; i<5; i++) {
//        texture[i].drawSubsection(100+30*i, 100, 10, 30, 0, 0);
//    }
...
void testApp::setCurrentTopRangeCorVectorHour(){
    currentTopRangeCorVectorHour.clear();
    vector<float> instantCor;
    for (int j = 0; j<csvOwnerList.size(); j++) {
        instantCor.clear();
        for (int i = 0; i<24; i++) {
            ...
            float referenceVar;
            float cov;
            targetVar = varTopRange(getGenreRateHour(j, i));
            referenceVar = varTopRange(getGenreRateTotal(j));
            cov = covTopRange(getGenreRateHour(j, i), getGenreRateTotal(j));
            instantCor.push_back(corTopRange(targetVar, referenceVar, cov));
        }
        currentTopRangeCorVectorHour.push_back(instantCor);
    }
}

vector< vector<float> > testApp::getTwentyFourHourBarGraphVector(int csvOwnerID){
    vector< vector<float> > twentyFourHour;
    //描画するベクターをつくる。
    hourBarVector.clear();
    for (int i = 0; i<24; i++) {
        vector<float> instantOneHourBar;
        for (int j = 0; j<csvGenreNameListVector[csvOwnerID].numRows-1; j++) {
            ...
            float instantSumHour = 0;
            for (int k = 0; k<7; k++) {
                instantSumHour += csvGraphDataMatrixVector[csvOwnerID].getInt(j+1, 24*k+i+1);//最初の行と列がラベルなのでそれぞれ１足す;
            }
            instantOneHourBar.push_back(instantSumHour);
        }
        //一時間分の棒グラフの累積データを格納。
        twentyFourHour.push_back(instantOneHourBar);
    }
    return twentyFourHour;
}


vector<float> testApp::getGenreRateHour(int csvOwnerID, int hourNum){
    //0~1の比率ではなくただのカウントの内訳が返ってくる。
    int total = 0;
    int topRange = 0;
    vector<float> thisHourTopRangeDetail;
    vector<float> allThisHourDetail;
    
    allThisHourDetail = getTwentyFourHourBarGraphVector(csvOwnerID)[hourNum];
    for (int i = 0; i<allThisHourDetail.size(); i++) {
        total += allThisHourDetail[i];
        if (i< myControlPanel.getValueI("topRange")) {
            thisHourTopRangeDetail.push_back(allThisHourDetail[i]);
        }
    }
//    for (int i = 0; i<myControlPanel.getValueI("topRange"); i++) {
//        thisHourTopRangeDetail[i] /= total;
//    }
    return thisHourTopRangeDetail;
}
...
void testApp::drawWeekRatio(string weekName, string csvOwnerName){
    ...
    //曜日毎のtopRangeの閉める割合を円グラフで出す。
    //飛ばした方
    if (myControlPanel.getValueB("topRangeRatio")) {
        ofPushMatrix();
        if (myControlPanel.getValueB("showLarge")) {
            ofTranslate(beginBarPosX+ofGetWidth()/7.0f*posX+ofGetWidth()/7.0f/2.0f, ofGetHeight()*0.97-10);
        }else{
        ...
        for (int i = 0; i<myControlPanel.getValueI("topRange"); i++) {
            ...
            if (myControlPanel.getValueB("showLarge")) {
                myVectorGraphics.arc(0, 0, 40, 0, 360*thisDayOfTheWeekTopRangeDetail[i]/(float)total);
            }else{
        ...
        ofSetColor(127, 127, 127, 127);
        if (myControlPanel.getValueB("showLarge")) {
            myVectorGraphics.arc(0, 0, 40, 0, 360*topRange/(float)total);
        }else{
    ...
    if (myControlPanel.getValueB("showDetailLabel")) {
        ...
        myFontJapaneseLarge.drawString(strRatio, ofGetWidth()/7.0f/2.0f-myFontForTimeLabel.stringWidth(strRatio)/2, myFontForTimeLabel.stringHeight(strDetail));
    }
    if (myControlPanel.getValueB("showDetailLabel")) {
    ...
    if (myControlPanel.getValueB("showDetailLabel")) {
        ...
        myFontJapaneseLarge.drawString("相関係数:" + ofToString(thisDayOfTheWeekCor), ofGetWidth()/7.0f/2.0f-myFontJapaneseLarge.stringWidth("相関係数:" + ofToString(thisDayOfTheWeekCor))/2, myFontJapaneseLarge.stringHeight(strRatio)*2.5);
    }
    ofPopMatrix();
    //飛ばされた方
    ofPushMatrix();
...
void testApp::keyPressed(int key){
    if (myControlPanel.getValueI("graphType") == 0) {
        ...
        //累積棒グラフ
        //FIXME: OF_KEY_LEFTとOF_KEY_RIGHT以外はなるべく共通にしておく。ガイドメッセージも出るようにする。
        switch (key) {
            case OF_KEY_LEFT:
                printf("left\n");
                beginBarPosX -= ofGetWidth()/168/*(float)myControlPanel.getValueI("barNum")*/;
                if (beginBarPosX < 0) {
...
void testApp::mouseDragged(int x, int y, int button){
    ...
    //2つのスライダーのシンクを取る。
    //トップレンジの設定に変更があったら再計算すべきものを再計算する。
...
void testApp::mousePressed(int x, int y, int button){
    ...
    modeNum = currentPos;
    if (x >= animationButtonPos.x && y >= animationButtonPos.y && x <= animationButtonPos.x+animationButtonImg.width && y <= animationButtonPos.y+animationButtonImg.height && animationSwitch == false) {
        animationSwitch = true;
        animationFrameNum = 0;//0はラベルが入っている
        //TODO: animation
        vector<int> instantGenreSizeZeros;
        for (int i = 0; i<csvGenreNameListVector[myControlPanel.getValueI("csvOwner")].numRows-1; i++) {
            instantGenreSizeZeros.push_back(0);
        }
        animationVector.clear();
        for (int i = 0 ; i<168; i++) {
            animationVector.push_back(instantGenreSizeZeros);
        }
    }
}
